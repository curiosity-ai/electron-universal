...
import { AppFile, AppFileType, getAllAppFiles } from './file-utils.js';
import { AsarMode, detectAsarMode, generateAsarIntegrity, mergeASARs } from './asar-utils.js';
...
export const makeUniversalApp = async (opts: MakeUniversalOpts): Promise<void> => {
  ...
  try {
    ...
    for (const file of x64Files.filter((f) => f.type === AppFileType.PLAIN)) {
      try {
        if (
          (await fs.pathExists(path.resolve(opts.x64AppPath, file.relativePath))) &&
          (await fs.pathExists(path.resolve(opts.arm64AppPath, file.relativePath)))
        ) {
          ...
          if (x64Sha !== arm64Sha) {
            d(
              'SHA for file',
              file.relativePath,
              `does not match across builds ${x64Sha}!=${arm64Sha}`,
            );
        ...
        throw new Error('failed to compare ' + file.relativePath);
      }
    }
    ...
    for (const machOFile of x64Files.filter((f) => f.type === AppFileType.MACHO)) {
      try {
        if (
          (await fs.pathExists(path.resolve(tmpApp, machOFile.relativePath))) &&
          (await fs.pathExists(path.resolve(opts.arm64AppPath, machOFile.relativePath)))
        ) {
